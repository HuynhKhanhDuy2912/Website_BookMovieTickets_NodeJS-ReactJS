...
import {
  ...
  UserX, Users, LayoutGrid, Filter, Crown, Globe
} from "lucide-react";
...
export default function Tickets() {
  ...
  // --- 3. XỬ LÝ KHI CHỌN SUẤT CHIẾU (ĐÃ FIX LỖI CRASH) ---
  useEffect(() => {
    ...
    // A. Lọc ghế đã bán (ĐÃ SỬA LỖI DÒNG 184)
    const taken = tickets
      .filter(t => {
        // 👇 FIX QUAN TRỌNG: Nếu vé bị null showtime thì bỏ qua ngay
        ...
        const taken = tickets
          .filter(t => {
            // 1. Bỏ qua vé lỗi hoặc đã hủy
            if (!t.showtime || t.status === 'cancelled') return false;

            // 2. Lấy ID suất chiếu của vé (Xử lý cả trường hợp populate hoặc chưa populate)
            const ticketShowtimeId = t.showtime._id
              ? t.showtime._id.toString()
              : t.showtime.toString();

            // 3. So sánh chính xác
            return ticketShowtimeId === currentShowtimeId;
          })
          .flatMap(t => {
            // 4. Lấy ghế từ mọi nguồn có thể (Mảng seats hoặc seatNumber lẻ)
  ...
  // --- FILTER TICKETS LIST (ĐÃ FIX LỖI CRASH TƯƠNG TỰ) ---
  const filteredTickets = formData.showtime
    ? tickets.filter(t => {
      if (!t.showtime) return false; // 👇 Fix thêm ở đây cho chắc
      const tShowtimeId = (t.showtime._id || t.showtime).toString();
      ...
      <div className="mt-4 border p-4 rounded-lg bg-gray-50 overflow-x-auto">
        ...
        <div className="flex flex-col gap-2 items-center min-w-max mx-auto">
            ...
            return (
              <div key={row} className="flex gap-2 items-center">
                ...
                {cols.map(col => {
                  ...
                  const isHolding = holdingSeats.includes(seatName) && !isTaken && !isSelected;
                    ...
                    <button
                      ...
                      className={`
                          ...
                          : isSelected
                            ...
                            : isHolding
                              ? 'bg-blue-100 border border-blue-400 text-blue-600 animate-pulse'
                              : isRowVip
                                ? 'bg-yellow-100 border border-yellow-400 text-yellow-700 hover:bg-yellow-200'
                                : 'bg-white border border-gray-300 hover:border-blue-400 hover:text-blue-500'
                        }
                               `}
                      title={`${seatName} ${isRowVip ? '(VIP)' : ''} ${isHolding ? '- Khách đang chọn' : ''}`}
                    >
                      {isRowVip && !isTaken && !isSelected && !isHolding && <Crown size={10} className="absolute -top-1.5 -right-1.5 text-yellow-600 drop-shadow-sm bg-white rounded-full p-[1px]" />}
                      {seatName}
                    </button>
                  )
                })}
          ...
          <div className="flex gap-4 mt-6 text-xs justify-center flex-wrap text-gray-600 border-t pt-2 w-full">
            ...
            <div className="flex items-center gap-1"><span className="w-3 h-3 bg-amber-600 rounded"></span> Bạn chọn</div>
            <div className="flex items-center gap-1"><span className="w-3 h-3 bg-blue-200 border border-blue-400 rounded"></span> Khách đang giữ</div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 bg-amber-600 hover:bg-amber-700 text-white p-3 rounded font-bold shadow-lg shadow-amber-600/30 transition disabled:bg-gray-300 flex justify-center items-center gap-2"
              >
