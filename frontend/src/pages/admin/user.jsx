...
import { getAllUsers, createUser, updateUser, toggleBlockUser } from "../../api/userService";
import {
  Loader2, SquarePen, User, Shield, Phone, Mail, Camera, Lock, Unlock, Ban
} from "lucide-react";
...
export default function Users() {
  ...
  // 🔥 HÀM XỬ LÝ CHẶN / BỎ CHẶN
  const handleToggleBlock = async (user) => {
      const actionName = user.isBlocked ? "BỎ CHẶN" : "CHẶN";
      if (!window.confirm(`Bạn có chắc muốn ${actionName} người dùng này không?`)) return;

      try {
          await toggleBlockUser(user._id); // Gọi API put /block/:id
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="bg-white shadow-md rounded-lg p-6 mb-8 border border-violet-100">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">{editingId ? "Mật khẩu mới" : "Mật khẩu"}</label>
              <div className="relative">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {users.map(user => (
            <div key={user._id} className={`bg-white border p-4 rounded-lg shadow-sm hover:shadow-md transition flex items-center gap-4 relative group ${user.isBlocked ? 'bg-red-50 border-red-200' : ''}`}>
              
              {/* Overlay nếu bị Block */}
              {user.isBlocked && (
                  <div className="absolute top-2 right-2 text-red-500 font-bold text-xs flex items-center gap-1 bg-white px-2 py-1 rounded-full shadow-sm border border-red-100">
                      <Ban size={12}/> Bị Chặn
                  </div>
              )}

              {/* Avatar */}
              <div className={`w-16 h-16 rounded-full overflow-hidden border shrink-0 ${user.isBlocked ? 'border-red-300 grayscale' : 'border-gray-200'}`}>
                <img src={user.avatar || "https://via.placeholder.com/150"} alt={user.name} className="w-full h-full object-cover" />
              </div>

              {/* Info */}
              <div className="flex-1 min-w-0">
                <h4 className={`font-bold truncate ${user.isBlocked ? 'text-gray-500 line-through' : 'text-gray-800'}`}>{user.name}</h4>
              ...
              <div className="flex flex-col gap-2">
                {/* Nút Sửa */}
                <button onClick={() => handleEdit(user)} className="p-2 bg-yellow-50 text-yellow-600 rounded hover:bg-yellow-100 transition" title="Sửa thông tin">
                  <SquarePen size={16} />
                </button>
                
                {/* 🔥 NÚT CHẶN / BỎ CHẶN */}
                {user.role !== 'admin' && ( // Không cho chặn Admin
                    <button 
                        onClick={() => handleToggleBlock(user)} 
                        className={`p-2 rounded transition ${
                            user.isBlocked 
                            ? 'bg-green-50 text-green-600 hover:bg-green-100' // Nút Mở Khóa
                            : 'bg-red-50 text-red-600 hover:bg-red-100'       // Nút Khóa
                        }`}
                        title={user.isBlocked ? "Mở khóa tài khoản" : "Chặn tài khoản"}
                    >
